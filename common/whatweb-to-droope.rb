# this needs to be in ruby because the orig format is in ruby.
require 'nokogiri'
...
export_files = {}
i = 0
versions.each do |version_nb,files|
  files.each do |file|
    filename = file[0]
    file_hash = file[1]
    if !export_files.has_key?(filename)
      export_files[filename] = []
    end
    export_files[filename] << {'version' => version_nb, 'hash' => file_hash}
  end
  i += 1
end
...
xml.cms {
  xml.files {
  export_files.each do |filename,file_version_hashes|
    xml.file(:url => filename) {
      file_version_hashes.each do |version_hash|
        xml.version(:nb => version_hash['version'], :md5 =>
                    version_hash['hash'])
      end
    }
  end
